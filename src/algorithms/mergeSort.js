...
import { setCurrentMergeX } from "../redux/reducers/merge";
...
function mergeSortHelper(array,toDispatch,start,end,obj){
      ...
      if(actualFirst.length+actualSecond.length===obj.array.length) isFinalMessage=true;

      return actualSort(actualFirst,actualSecond,toDispatch,obj,start,end,isFinalMessage);

}

function actualSort(first,second,toDispatch,obj,start,end,isFinalMessage){
    let sortedArray=[];
    ...
    while(first.length && second.length){
        ...
        if(isFinalMessage) toDispatch.push([true,indexToPush-1]);
    }
...
function handleDispatch(toDispatch, dispatch, array, speed) {
    ...
    let dispatchFunction = toDispatch[0].length > 3 ? setArray : 
                           toDispatch[0].length === 3 && typeof toDispatch[0][2] === "boolean" || toDispatch[0].length === 0 ? setCurrentSwapper : 
