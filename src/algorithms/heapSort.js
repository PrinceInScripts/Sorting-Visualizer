...
import { setCurrentHeap } from "../redux/reducers/heap";
...
function handleDispatch(toDispatch,dispatch,array,speed){
    if(!toDispatch.length){
        dispatch(setCurrentHeap(array.map((num,index)=>index)));
        setTimeout(()=>{
    ...
    let dispatchFunction=toDispatch[0].length>3 ?setArray:
                         toDispatch[0].length===3 && typeof toDispatch[0][2]==="boolean" || !toDispatch[0].length ?setCurrentSwapper:
